...
def build_model(cfg: dict) -> DualStreamEncodecSeparator:
    ...
    return DualStreamEncodecSeparator(
        ...
        num_stems=int(model.get("num_stems", 4)),
        residual_predictor=str(model.get("residual_predictor", "mtp")),
        mtp_layers=int(model.get("mtp_layers", 2)),
        mtp_heads=int(model.get("mtp_heads", 8)),
        mtp_ffn_dim=int(model.get("mtp_ffn_dim", max(4 * int(model["d_model"]), int(model["ffn_dim"])))),
    )
